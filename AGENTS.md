...
- `CHANGELOG.md` is generated by `git-cliff` from `.github/cliff.toml` and regenerated on each release (`release.yml` uploads it as the `changelog` artifact, then the Linux bundle build copies it into place). Keep `.github/cliff.toml` in sync with `.github/cliff-release.toml` so the per-release notes and the full changelog stay consistent.
