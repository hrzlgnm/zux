...
- The man page ships as `/usr/share/man/man1/zux.1`, installed via `bundle.linux.deb.files` and `bundle.linux.rpm.files` in `src-tauri/tauri.conf.json` (source `docs/zux.1`) and via `packaging/aur/generate-zux.sh`. Keep `docs/zux.1` in sync with `README.md`.
