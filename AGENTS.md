...
- `CHANGELOG.md` and the `vMAJOR.MINOR.PATCH` tags are managed by release-please (`release-please-config.json` + `.release-please-manifest.json`); never edit the changelog or push version tags manually. Version bumps land in the Release PR for `package.json`, `src-tauri/tauri.conf.json`, and `src-tauri/Cargo.toml`, and a lock-sync job keeps `src-tauri/Cargo.lock` exact on the Release PR itself, so builds use the tag checkout directly. Merge the `release-please` Release PR to tag and open a draft release, wait for the release workflow to upload all assets into the draft, then publish it manually so downstream `release: [released]` packaging workflows fire.
