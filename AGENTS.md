...
- The deb changelog ships as `/usr/share/doc/zux/changelog.gz`, generated by Tauri from `CHANGELOG.md` via `bundle.linux.deb.changelog` in `src-tauri/tauri.conf.json`. Do **not** point it at a Debian-format changelog — Tauri gzips the upstream markdown as-is.
- The rpm ships the same changelog as `/usr/share/doc/zux/changelog` via `bundle.linux.rpm.files` in `src-tauri/tauri.conf.json`.
- The man page (`docs/zux.1`) ships as `/usr/share/man/man1/zux.1`, installed via `bundle.linux.deb.files` and `bundle.linux.rpm.files` in `src-tauri/tauri.conf.json` (source `../docs/zux.1`) and via `packaging/aur/generate-zux.sh`. It must document **every** flag from the `clap::Parser` `Cli` in `src-tauri/src/lib.rs` (including the Linux-only NVIDIA/dmabuf flags) and stay in sync with the `Command line options` section of `README.md` — when the CLI or the README changes, update both.
- `CHANGELOG.md` is generated by `git-cliff` from `.github/cliff.toml` and regenerated on each release (`release.yml` uploads it as the `changelog` artifact, then the Linux bundle build copies it into place). `.github/cliff.toml` (full changelog) and `.github/cliff-release.toml` (release notes) intentionally use different commit-group mappings; only their shared settings (repo URLs, `tag_pattern`) need to match. **Never edit `CHANGELOG.md` by hand** — it is automated; only a correct conventional commit drives its contents, and the release workflow regenerates it. Do not add manual "Unreleased" entries.
