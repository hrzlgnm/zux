...
function typeId(st: string) {
  return `type:${st}`
}
function instId(name: string, st: string) {
  return `inst:${name}:${st}`
}
function hostId(h: string) {
  return `host:${h}`
}
function addrId(a: string) {
  return `addr:${a}`
}
    ...
    case 'service-type-added': {
      ...
      graphNodes.update((m) => {
        if (!m.has(id)) {
          m.set(id, {
            id,
            label: st.replace('.local.', '').replace(/\.$/, ''),
            group: 'service-type',
            shape: 'diamond',
            size: 25,
            color: '#4fc3f7',
    ...
    case 'service-added': {
      ...
      graphNodes.update((m) => {
        ...
        if (!m.has(hId)) {
          m.set(hId, {
            id: hId,
            label: d.hostname.replace('.local.', ''),
            group: 'host',
            shape: 'square',
            size: 20,
            color: '#ffb74d',
